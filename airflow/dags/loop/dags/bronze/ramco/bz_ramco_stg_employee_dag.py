...
dag_id = "bz_ramco_stg_employee_dag"
...
def stg_to_bz_ramco_employee_ingestion():
    """DAG to load employee master data from MinIO to bronze and archive input file."""
    ...
    def list_files():
        try:
            ...
            process_path = nfs_cfg["directories"]["ramco"]["process_directory"]
            file_prefix = nfs_cfg["directories"]["ramco"]["file_prefix"]
    ...
    def insert_batch_data(batch_ids: list, files: list):
        try:
            ...
            jinja_env = Environment(loader=FileSystemLoader(SQL_TEMPLATE_DIR))
            template = jinja_env.get_template("bz_ramco_stg_employee.sql.jinja")
    ...
    def archive_file(files: list):
        try:
            ...
            nfs_cfg = env_cfg["nfs"]
            process_dir = nfs_cfg["directories"]["ramco"]["process_directory"].rstrip("/")
            archive_dir = nfs_cfg["directories"]["ramco"]["archive_directory"].rstrip("/")
    ...
    trigger_silver_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id="sz_ramco_bz_employee_dag",
...
# --- DAG Instance ---
stg_to_bz_ramco_employee_ingestion = stg_to_bz_ramco_employee_ingestion()

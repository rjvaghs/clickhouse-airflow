...
dag_id = "bz_biomax_stg_employee_daily_attendance_dag"
...
@dag(
    ...
    catchup=False,
    tags=["clickhouse", "bronze", "biomax"],
)
def stg_to_bz_biomax_employee_daily_attendance_ingestion():
    ...
    def list_files():
        try:
            ...
            process_path = nfs_cfg["directories"]["biomax"]["process_directory"]
            file_prefix = nfs_cfg["directories"]["biomax"]["file_prefix"]
    ...
    def insert_batch_data(batch_ids: list, files: list):
        try:
            ...
            template = jinja_env.get_template(
                "bz_employee_stg_employee_daily_attendance.sql.jinja"
            )
    ...
    def archive_file(files: list):
        try:
            ...
            nfs_cfg = env_cfg["nfs"]
            process_dir = nfs_cfg["directories"]["biomax"]["process_directory"].rstrip("/")
            archive_dir = nfs_cfg["directories"]["biomax"]["archive_directory"].rstrip("/")
    ...
    trigger_silver_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id="sz_biomax_bz_employee_daily_attendance_dag",
...
stg_to_bz_biomax_employee_daily_attendance_ingestion = stg_to_bz_biomax_employee_daily_attendance_ingestion()
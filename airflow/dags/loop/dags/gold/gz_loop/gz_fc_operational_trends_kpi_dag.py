...
dag_id = "gz_fc_operational_trends_kpi_dag"
...
def gz_fc_operational_trends_kpi_dag():
    ...
    def get_affected_log_dates():
        try:
            ...
            logging.info(f"FC operational trends affected log_dates: {sorted_dates}")
            return sorted_dates
    ...
    @task()
    def insert_delta_gz_fc_operational_trends_kpi(log_dates):
        if not log_dates:
            logging.info("No affected log_dates for gz_fc_operational_trends_kpi.")
            return

        try:
            clickhouse_conn_id_local = cfg_loader.load_env_config()["clickhouse"]["conn_id"]

            target_table_key = "fc_operations"
            ...
            template = jinja_env.get_template("gz_fc_operational_trends_kpi.sql.jinja")
    ...
    insert = insert_delta_gz_fc_operational_trends_kpi(affected_log_dates)
...
gz_fc_operational_trends_kpi_dag = gz_fc_operational_trends_kpi_dag()
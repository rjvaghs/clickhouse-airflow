...
dag_id = "gz_input_dashboard_kpi_dag"
...
def gz_input_dashboard_kpi_dag():
        ...
        try:
            ...
            queries = [
                (
                    ...
                    FROM {silver_db}.{source_table_employee_attendance}
                ...
                (
                    f"""
                    SELECT DISTINCT log_date
                    FROM {silver_db}.{source_table_resource_level_productivity}
                    WHERE event_ts > toDateTime('%(wm)s')
                    """,
            ...
            logging.info(f"Input dashboard affected log_dates: {result}")
            return result
    ...
    @task()
    def insert_delta_gz_input_dashboard_kpi(log_dates):
        if not log_dates:
            logging.info("No affected log_dates to process for gz_input_dashboard_kpi.")
            return

        try:
            clickhouse_conn_id_local = cfg_loader.load_env_config()["clickhouse"]["conn_id"]

            target_table_key = "input_dashboard"
            ...
            template = jinja_env.get_template("gz_input_dashboard_kpi.sql.jinja")

            sql = template.render(
                target_db=target_db,
                target_table=target_table,
                source_db=silver_db,
                source_table_employee_attendance=source_table_employee_attendance,
                source_table_da_productivity=source_table_da_productivity,
                source_table_shopper_productivity=source_table_shopper_productivity,
                source_table_vehicle_productivity=source_table_vehicle_productivity,
                source_table_resource_productivity_pc=source_table_resource_productivity_pc,
                source_table_resource_level_productivity=source_table_resource_level_productivity,
                source_table_fc_productivity=source_table_fc_productivity,
    ...
    insert = insert_delta_gz_input_dashboard_kpi(affected_log_dates)
...
gz_input_dashboard_kpi_dag = gz_input_dashboard_kpi_dag()
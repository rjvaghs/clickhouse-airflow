...
dag_id = "stg_biomax_sftp_employee_daily_attendance_dag"
...
@dag(
    ...
    schedule=CronTriggerTimetable("0 2 * * *", timezone=local_tz),   # 2 AM IST daily
    catchup=False,
    default_args=default_args,
    tags=["sftp", "nfs", "biomax", "bronze"],
)
def sftp_biomax_ingestion():
    ...
    def list_sftp_files():
        ...
        """
        try:
            sftp_cfg = env_cfg["sftp-biomax"]
            conn_id = sftp_cfg["conn_id"]
            remote_dir = sftp_cfg["directory"]
            file_prefix = env_cfg["nfs"]["directories"]["biomax"]["file_prefix"]
    ...
    def copy_to_nfs(files: list):
        ...
        try:
            ...
            nfs_cfg = env_cfg["nfs"]["directories"]["biomax"]
    ...
    trigger_bronze_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id="bz_biomax_stg_employee_daily_attendance_dag",
...
sftp_biomax_ingestion_dag = sftp_biomax_ingestion()
